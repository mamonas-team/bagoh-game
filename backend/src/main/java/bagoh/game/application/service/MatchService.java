...
import bagoh.game.application.dto.domainDto.BidValues;
...
public class MatchService {
    ...
    public void fazerAposta2(Long idJogador, Bid aposta, String ordem) {
        int apostaQuantidade = aposta.getQuantity();
        int apostaValor = aposta.getValue().getNumericValue();

        validarAposta(aposta, apostaQuantidade, apostaValor);
...
//        }
    ...
    //fazerAposta(idJogador, aposta, ordem){
    //  validacoesInicias();
    //  validarAposta();
    //  adicionarHistorico();
    //  removerDadoJogador();
    //  chamarVezDoProximoJogador();
    // }

    private void validarAposta(Bid aposta, int apostaQuantidade, int apostaValor) {
        int bago = 1;
        int sena = 6;
        if (apostaQuantidade > totalDicesMatch){
            aposta.setInvalidReason("Aposta maior que o limite de dados na mesa");
        } else if (apostaQuantidade <= 0 || apostaValor < bago || apostaValor > sena){
            aposta.setInvalidReason("Aposta inválida. Os valores dos dados vão de 1 a 6 apenas");
        } else {
            aposta.setValid(true);
        }
    }
